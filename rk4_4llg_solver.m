...
if config.dimensionlessLLG
    Bk = [1 * (params.WFL < params.LFL) * Bk, ...
        1 * (params.WFL > params.LFL) * Bk, 0];
    tau_c = (params.g * Bk(2 * (params.WFL > params.LFL) ...
...
for step_idx = 1:n_steps
    ...
    kk1 = evaluate_rhs(mm1, Bk, params, config, constants, scal);
    kk2 = evaluate_rhs(mm1 + kk1 * ts1 / 2, ...
        Bk, params, config, constants, scal);
...
function kk = evaluate_rhs(mmm, Bk, params, config, constants, scal)
[Beff, sttdlt, sttflt, sotdlt, sotflt] = field_eta( ...
    mmm, Bk, params.Demag_, params.Hext, params.jc_STT, ...
...
dmdt = LLG_solver(params.alp, mmm, Beff, params.polSOT, params.PolSTT, ...
